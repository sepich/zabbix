...
def main():
  #load json data
  try:
    f = urllib.urlopen("http://localhost:9200/_cluster/health")
    health = json.loads( f.read() )
    f = urllib.urlopen("http://localhost:9200/_nodes/_local/stats?all=true")
  ...
  out += getKeys(all,traps2)    #getting stats  values
  ...
  if len(sys.argv)>1 and sys.argv[1]=='jvm.uptime_in_millis':
    ...
    print all['jvm']['uptime_in_millis']
